...
def create_message(sock, account_name='Guest'):
    ...
    if message == '!!!':
        sock.close()
        log.info(f'Завершено по команде пользователя')
        print(f'До свидания')
        time.sleep(2)
        sys.exit(0)
...
def arg_parser():
    ...
    parser.add_argument('-m', '--mode', default='listen', nargs='?', help='send or listen mode')
...
def main():
    ...
    if client_mode != 'send' and client_mode != 'listen':
        log.warning(f'Указанный режим работы не поддерживается! Укажите send или listen')
        sys.exit(1)
    ...
    if client_mode == 'send':
        print(f'Режим отправки сообщений.')
        while True:
            try:
